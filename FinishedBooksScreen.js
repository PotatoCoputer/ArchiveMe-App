...
let storage;
try {
  storage = require('firebase/storage').getStorage?.() || undefined;
} catch {}
const { getDownloadURL, ref: storageRef } = (() => {
  try {
    const m = require('firebase/storage');
    return { getDownloadURL: m.getDownloadURL, ref: m.ref };
  } catch {
    return { getDownloadURL: null, ref: null };
  }
})();
...
export default function FinishedBooksScreen() {
    ...
    const unsub = onSnapshot(collection(db, 'users', uid, 'books'), (qs) => {
      const data = qs.docs
        .map((d) => {
          ...
          const isFinished = (total > 0 && read >= total) || b.status === 'อ่านจบแล้ว';
          return { ...b, _isFinished: isFinished };
        })
        .filter((b) => b._isFinished);
      setRawBooks(data);
      setLoading(false);
      setRefreshing(false);
    });
  ...
  // helpers 
  const toMillis = (tsPrefer, tsFallback) => {
    const ts = tsPrefer || tsFallback;
    if (!ts) return 0;
  ...
  // อัปเดตปก
  const findCoverFromStorage = async (b) => {
    try {
      if (!getDownloadURL || !storageRef || !storage || !b.coverPath) return null;
      const ref = storageRef(storage, b.coverPath);
  ...
  const findCoverFromGoogleBooks = async (b) => {
    try {
      let url = '';
      const isbn = (b.isbn || b.ISBN || '').toString().trim();
      if (isbn) {
        const r = await fetch(
      ...
      if (!url && b.title) {
        const r = await fetch(
          `https://www.googleapis.com/books/v1/volumes?q=intitle:${encodeURIComponent(b.title)}`
        );
        const j = await r.json();
        url =
          j?.items?.[0]?.volumeInfo?.imageLinks?.thumbnail ||
      ...
      if (!url) return null;
      return String(url).replace(/^http:\/\//, 'https://').replace('&edge=curl', '');
    } catch {
      return null;
    }
  };
  const resolveCovers = async (books) => {
    if (!uid || resolvingRef.current) return;
    resolvingRef.current = true;
    try {
      const updates = {};
      for (const b of books) {
        const existing = (b.coverUri || '').trim();
        if (existing) {
          updates[b.id] = existing;
          continue;
        }
        let found = await findCoverFromStorage(b);
        if (!found) found = await findCoverFromGoogleBooks(b);
        if (found) {
          updates[b.id] = found;
          try {
            await updateDoc(doc(db, 'users', uid, 'books', b.id), { coverUri: found });
          } catch {}
        }
      }
      if (Object.keys(updates).length) setCoverMap((prev) => ({ ...prev, ...updates }));
    } finally {
      resolvingRef.current = false;
    }
  };
  useEffect(() => {
    if (rawBooks?.length) resolveCovers(rawBooks);
  }, [rawBooks]);

  // หมวด + กรอง + เรียง
  const categories = useMemo(() => {
    const set = new Set();
    rawBooks.forEach((b) => set.add((b.category || 'ไม่ระบุหมวดหมู่').trim()));
  ...
  const books = useMemo(() => {
    const filtered =
      selectedCategory === 'ทั้งหมด'
        ? rawBooks
        : rawBooks.filter((b) => (b.category || 'ไม่ระบุหมวดหมู่').trim() === selectedCategory);

    return [...filtered].sort((a, b) => {
      const am = toMillis(a.finishedAt, a.createdAt);
      const bm = toMillis(b.finishedAt, b.createdAt);
  ...
  const renderItem = ({ item: b }) => {
    const coverUrl = coverMap[b.id] || b.coverUri;
    return (
      <View style={styles.card}>
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.titleRow}>
            ...
            >
              <Text style={[styles.badgeText, { color: '#6EE7B7' }]}>อ่านจบแล้ว</Text>
            </View>
          </View>
          ...
          <View style={styles.metaBlock}>
            ...
            <Text style={styles.metaDate}>
              จบเมื่อ {formatDate(b.finishedAt) || formatDate(b.updatedAt) || '-'}
            </Text>
          </View>
        </View>
      </View>
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      >
        ...
        <Text style={styles.title}>หนังสือที่อ่านจบแล้ว</Text>
        <View style={{ width: 56 }} />
      </LinearGradient>
      ...
      <View style={styles.toolbar}>
        <Text style={styles.countText}>
          ...
          {selectedCategory !== 'ทั้งหมด' ? ` • หมวด: ${selectedCategory}` : ''}
        </Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptyText}>ยังไม่มีหนังสือที่อ่านจบ</Text>
            <Text style={styles.emptySub}>เมื่ออ่านจบ ระบบจะย้ายเล่มมาแสดงที่นี่อัตโนมัติ</Text>
          </View>
        }
        refreshControl={
          ...
          <View style={styles.modalCard}>
            ...
            <ScrollView style={{ maxHeight: 360 }}>
              {categories.map((cat) => (
                ...
                >
                  <Text
                    style={[styles.optionText, selectedCategory === cat && styles.optionTextActive]}
                    numberOfLines={1}
                  >
                    {cat}
                  </Text>
        ...
        <View style={styles.modalWrap}>
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              ...
              <Text style={styles.modalTitle}>เรียงตามวันที่จบ/เพิ่ม</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  catTxt: { color: UI.cyan, fontSize: 12, maxWidth: 140 },
